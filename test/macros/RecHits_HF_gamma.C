// Commands executed in a GLOBAL scope, e.g. created hitograms aren't erased...
{
   ...
   TFile f1("HcalRecHitValidationHF_gamma_210pre6.root");
   TFile f2("HcalRecHitValidationHF_gamma_210pre9.root");
   ...
   f1_hist2[0]->GetXaxis()->SetTitle("HF gamma RecHits (GeV) energy vs SimHits energy (photo electrons)");
   ...
   for (int i = 0; i < Nhist1; i++){
     ...
     if(i <= 1 )   f1_hist1[i]->SetStats(kTRUE);   // stat
  ...
  for (int i = 0; i < Nhist2; i++){
    ...
    TLegend *leg = new TLegend(0.43, 0.82, 0.63, 0.97, "","brNDC");    
     leg->SetBorderSize(2);
     leg->SetFillStyle(1001); 
     leg->AddEntry(f1_histp[i],"CMSSW_210pre6","pl");
     leg->AddEntry(f2_histp[i],"CMSSW_210pre9","pl");
  ...
  for (int i = 0; i < Nprof; i++){
    ...
    TLegend *leg = new TLegend(0.78, 0.82, 0.98, 0.97, "","brNDC");    
     leg->SetBorderSize(2);
     leg->SetFillStyle(1001); 
     leg->AddEntry(f1_prof[i],"CMSSW_210pre6","pl");
     leg->AddEntry(f2_prof[i],"CMSSW_210pre9","pl");
