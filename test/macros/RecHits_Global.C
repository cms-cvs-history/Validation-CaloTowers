// Commands executed in a GLOBAL scope, e.g. created hitograms aren't erased...
{
   ...
   TFile f1("HcalRecHitValidationALL_206.root");
   TFile f2("HcalRecHitValidationALL_210pre6.root");
   ...
   f1_hist2[4]->GetXaxis()->SetTitle("HFL RecHits energy (GeV) vs SimHits energy (p.e)");
   f1_hist2[5]->GetXaxis()->SetTitle("HFS RecHits energy (GeV) vs SimHits energy (p.e.)");
   ...
   //
   f1_prof[0]->SetMaximum(60.);
   f1_prof[1]->SetMaximum(30.);
   f1_prof[2]->SetMaximum(20.);
  ...
  for (int i = 0; i < Nprof; i++){
    ...
    TLegend *leg = new TLegend(0.78, 0.82, 0.98, 0.97, "","brNDC");    
     ...
     leg->AddEntry(f1_prof[i],"CMSSW_206","pl");
     leg->AddEntry(f2_prof[i],"CMSSW_210pre6","pl");
  ...
  for (int i = 0; i < Nhist2; i++){
    ...
    TLegend *leg = new TLegend(0.48, 0.82, 0.68, 0.97, "","brNDC");    
     leg->SetBorderSize(2);
     leg->SetFillStyle(1001); 
     leg->AddEntry(f1_hist2[i],"CMSSW_206","l");
     leg->AddEntry(f2_hist2[i],"CMSSW_210pe6","l");
